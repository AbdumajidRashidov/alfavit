export type Locale = 'uz' | 'ru' | 'en'

export const translations = {
  en: {
    'nav.convert': 'Convert',
    'nav.reform': 'Reform',
    'nav.developers': 'Developers',
    'nav.telegram': 'Telegram',
    'nav.reach': 'Reach us',
    'nav.cta': 'Convert now',
    'hero.headlinePre': "Your words, in Uzbekistan's ",
    'hero.headlineEm': 'new alphabet.',
    'hero.headlinePost': '',
    'hero.desc': 'Instantly convert Uzbek text from Cyrillic or the old Latin into the reformed 2026 Latin script — free, in your browser, nothing uploaded.',
    'converter.inputLabel': 'Cyrillic or old Latin',
    'converter.outputLabel': 'New Latin',
    'converter.placeholder': 'Matn kiriting…',
    'converter.copy': 'Copy',
    'converter.copied': 'Copied',
    'converter.detected': 'Detected',
    'script.cyrillic': 'Cyrillic',
    'script.old-latin': 'Old Latin',
    'script.foreign': 'Text',
    'channels.title': 'Get Alfavit everywhere.',
    'channels.subtitle': 'One engine — every platform you write on.',
    'channels.group.desktop': 'Desktop',
    'channels.group.mobile': 'Mobile',
    'channels.group.more': 'More ways',
    'channels.open': 'Open',
    'channels.download': 'Download',
    'channels.mac.desc': 'Type anywhere — Uzbek Cyrillic or old-Latin converts to new-Latin as you go.',
    'channels.mac.note': 'Unsigned — if the first open is blocked, allow it in System Settings → Privacy & Security.',
    'channels.windows.desc': 'Type anywhere — Uzbek Cyrillic or old-Latin converts to new-Latin as you go.',
    'channels.windows.note': 'Unsigned — if SmartScreen blocks the first run, choose More info → Run anyway.',
    'channels.badge.new': 'New',
    'channels.web.name': 'Web app',
    'channels.telegram.name': 'Telegram bot',
    'channels.extension.name': 'Browser extension',
    'channels.api.name': 'API & SDK',
    'channels.mac.name': 'macOS',
    'channels.windows.name': 'Windows',
    'channels.ios.name': 'iOS',
    'channels.android.name': 'Android',
    'status.soon': 'Coming soon',
    'file.title': 'Convert a file',
    'file.instruction': 'Your file is converted in your browser — nothing is uploaded.',
    'file.choose': 'Choose a file (.txt, .srt, .docx)',
    'file.convert': 'Convert & download',
    'file.converting': 'Converting…',
    'file.done': 'Done — your download should start.',
    'file.unsupported': 'Unsupported file type. Use .txt, .srt, or .docx.',
    'file.error': 'Something went wrong converting that file. Please try another file.',
    'dev.title': 'Developers',
    'dev.intro': 'A free public API to convert Uzbek text to the new Latin script — no key required.',
    'dev.responseLabel': 'Response',
    'dev.rateLimit': 'Rate limit: 60 requests / minute per IP.',
    'nav.files': 'Files',
    'nav.apps': 'Apps',
    'reform.title': 'The 2026 reform',
    'reform.intro': 'In 2026 Uzbekistan reformed its Latin alphabet: the 1995 digraphs and apostrophe-letters became single letters.',
    'reform.changesLabel': 'Letter changes',
    'reform.spotlightsLabel': 'Letter by letter',
    'reform.sourcesLabel': 'Sources',
    'reform.law': 'Adopted by the Legislative Chamber on 7 July 2026 and approved by the Senate on 10 September 2026; it now awaits the President’s signature. The alphabet has 28 letters and one apostrophe sign (was 26 letters and 3 letter combinations).',
    'reform.why': 'Single letters remove clumsy digraphs and apostrophes — which broke software, URLs, and search — and align Uzbek with other Turkic Latin alphabets.',
    'footer.reform': 'The reform',
    'footer.developers': 'Developers',
    'footer.telegram': 'Telegram',
    'meta.home.title': 'Alfavit — Uzbek new Latin converter',
    'meta.home.desc': 'Convert Uzbek from Cyrillic or old Latin to the reformed 2026 new Latin — free, instant, on-device.',
    'meta.files.title': 'Convert files — Alfavit',
    'meta.files.desc': 'Convert .txt, .srt, and .docx files to the new Uzbek Latin — in your browser, nothing uploaded.',
    'meta.apps.title': 'Apps & channels — Alfavit',
    'meta.apps.desc': 'Get Alfavit as a web app, Telegram bot, browser extension, and public API — one engine, every platform.',
    'meta.dev.title': 'API & SDK — Alfavit',
    'meta.dev.desc': 'Free public API to convert Uzbek text to the new Latin script — no key required.',
    'meta.reform.title': 'The 2026 alphabet reform — Alfavit',
    'meta.reform.desc': 'Uzbekistan’s 2026 Latin alphabet reform, letter by letter: what changed, what the Senate approved on 10 September 2026, and the transition timeline.',
    'meta.privacy.title': 'Privacy — Alfavit',
    'meta.privacy.desc': 'Alfavit runs entirely on your device. We collect no data.',
    'faq.title': 'Frequently asked questions',
    'meta.guide.cyrillic.title': 'Cyrillic to new Latin — Alfavit',
    'meta.guide.cyrillic.desc': 'Convert Uzbek text from Cyrillic to the reformed 2026 new Latin, step by step.',
    'meta.guide.oldlatin.title': 'Old Latin to new Latin — Alfavit',
    'meta.guide.oldlatin.desc': 'Convert 1995 old-Latin digraphs and apostrophe-letters to the new Latin, step by step.',
    'guides.label': 'Guides',
    'guides.cyrillic': 'Cyrillic → new Latin',
    'guides.oldlatin': 'Old Latin → new Latin',
    'meta.alphabet.title': 'Uzbek alphabet (2026) — full reformed Latin chart | Alfavit',
    'meta.alphabet.desc': 'The complete reformed 2026 Uzbek Latin alphabet: all 28 letters with their old-Latin (1995) and Cyrillic equivalents, pronunciation, and examples.',
    'nav.alphabet': 'Alphabet',
    'footer.alphabet': 'Alphabet',
    'alphabet.title': 'The Uzbek alphabet',
    'alphabet.intro': 'The reformed 2026 Uzbek Latin alphabet has 28 letters and the tutuq belgisi (ʼ). Each letter is shown with its old-Latin (1995) form, Cyrillic equivalent, sound, and an example.',
    'alphabet.col.new': 'New (2026)',
    'alphabet.col.old': 'Old-Latin (1995)',
    'alphabet.col.cyrillic': 'Cyrillic',
    'alphabet.col.sound': 'Sound',
    'alphabet.col.example': 'Example',
    'alphabet.changedLabel': 'Changed',
    'alphabet.tutuq.label': 'Tutuq belgisi (ʼ)',
    'alphabet.tutuq.note': 'The apostrophe sign — separates vowels and marks a glottal stop. It is not one of the 28 letters.',
    'alphabet.loanword.note': 'In loanwords, Cyrillic ц is written s; Alfavit marks c as an alternative so you can choose.',
    // Chart block. uz and ru copy is a draft — native-speaker check before merge.
    'alphabet.chart.heading': 'Download the alphabet chart',
    'alphabet.chart.caption': 'A4 and print-ready — all 28 letters, with the four that changed marked.',
    'alphabet.chart.alt': 'Uzbek Latin alphabet chart (2026): all 28 letters with their old-Latin (1995) and Cyrillic equivalents, sounds and examples',
    'alphabet.chart.png': 'PNG image',
    'alphabet.chart.pdf': 'A4 PDF',
    'notfound.title': 'Page not found',
    'notfound.body': 'That address does not exist on alfavit.uz. It may have been a typo, or a link that has moved.',
    'meta.status.title': 'Has the Uzbek alphabet changed yet? — Alfavit',
    'meta.status.desc': 'Whether Uzbekistan\u2019s 2026 alphabet reform is in force: the law passed the Senate on 10 September 2026 and awaits the President\u2019s signature. Checked and dated.',
    'status.title': 'Has the alphabet changed yet?',
    'status.checked': 'Status checked on',
    'status.timeline': 'How it got here, and what happens next',
    'status.sources': 'Sources',
    'alphabet.count.heading': 'How many letters are in the Uzbek alphabet?',
    'alphabet.count.body': 'The reformed 2026 Uzbek Latin alphabet has 28 letters, plus the tutuq belgisi (\u02bc), which is a sign rather than a letter. The 1995 alphabet had 26 letters and 3 digraphs (sh, ch, ng); the reform turned sh, ch, g\u02bb and o\u02bb into the single letters \u015f, \u00e7, \u011f and \u00f6, and ng stopped counting as a separate letter.',
    'reform.timelineLabel': 'Timeline',
    'reform.updated': 'Updated 10 September 2026',
    'reform.ngNote': 'The combination ng is no longer listed as a separate letter; it stays in spelling as n + g.',
    'news.senate': '10 September: the Senate approved the 28-letter alphabet. What changes →',
    'meta.guide.keyboard.title': 'How to type Ş, Ç, Ö, Ğ — Alfavit',
    'meta.guide.keyboard.desc': 'Type the four new Uzbek letters on Mac, Windows, iPhone and Android: copy buttons, keyboard layouts, shortcuts — or let Alfavit convert as you type.',
    'guides.keyboard': 'How to type ş ç ö ğ',
    'converter.share': 'Share',
  },
  uz: {
    'nav.convert': 'Oʻgirish',
    'nav.reform': 'Islohot',
    'nav.developers': 'Dasturchilar',
    'nav.telegram': 'Telegram',
    'nav.reach': 'Aloqa',
    'nav.cta': 'Boshlash',
    'hero.headlinePre': 'Soʻzlaringiz — Oʻzbekistonning ',
    'hero.headlineEm': 'yangi alifbosida.',
    'hero.headlinePost': '',
    'hero.desc': 'Oʻzbek matnini kirill yoki eski lotindan 2026-yilgi yangilangan lotin yozuviga bir zumda oʻgiring — bepul, brauzeringizda, hech narsa yuklanmaydi.',
    'converter.inputLabel': 'Kirill yoki eski lotin',
    'converter.outputLabel': 'Yangi lotin',
    'converter.placeholder': 'Matn kiriting…',
    'converter.copy': 'Nusxa olish',
    'converter.copied': 'Nusxa olindi',
    'converter.detected': 'Aniqlandi',
    'script.cyrillic': 'Kirill',
    'script.old-latin': 'Eski lotin',
    'script.foreign': 'Matn',
    'channels.title': 'Alfavit — hamma joyda.',
    'channels.subtitle': 'Bir tizim — yozadigan har bir platformangizda.',
    'channels.group.desktop': 'Ish stoli',
    'channels.group.mobile': 'Mobil',
    'channels.group.more': 'Boshqa yoʻllar',
    'channels.open': 'Ochish',
    'channels.download': 'Yuklab olish',
    'channels.mac.desc': 'Istalgan ilovada yozing — kirill yoki eski lotin yozuvingiz shu zahoti yangi lotinga oʻgiriladi.',
    'channels.mac.note': 'Imzosiz — ilk ochish bloklansa, System Settings → Privacy & Security’da ruxsat bering.',
    'channels.windows.desc': 'Istalgan ilovada yozing — kirill yoki eski lotin yozuvingiz shu zahoti yangi lotinga oʻgiriladi.',
    'channels.windows.note': 'Imzosiz — SmartScreen ilk ishga tushirishni bloklasa, More info → Run anyway’ni tanlang.',
    'channels.badge.new': 'Yangi',
    'channels.web.name': 'Veb ilova',
    'channels.telegram.name': 'Telegram bot',
    'channels.extension.name': 'Brauzer kengaytmasi',
    'channels.api.name': 'API va SDK',
    'channels.mac.name': 'macOS',
    'channels.windows.name': 'Windows',
    'channels.ios.name': 'iOS',
    'channels.android.name': 'Android',
    'status.soon': 'Tez orada',
    'file.title': 'Faylni oʻgirish',
    'file.instruction': 'Fayl brauzeringizda oʻgiriladi — hech narsa yuklanmaydi.',
    'file.choose': 'Fayl tanlang (.txt, .srt, .docx)',
    'file.convert': 'Oʻgirish va yuklab olish',
    'file.converting': 'Oʻgirilmoqda…',
    'file.done': 'Tayyor — yuklab olish boshlanadi.',
    'file.unsupported': 'Qoʻllab-quvvatlanmaydigan fayl turi. .txt, .srt yoki .docx ishlating.',
    'file.error': 'Faylni oʻgirishda xatolik yuz berdi. Boshqa fayl bilan urinib koʻring.',
    'dev.title': 'Dasturchilar',
    'dev.intro': 'Oʻzbek matnini yangi lotin yozuviga oʻgirish uchun bepul ochiq API — kalit shart emas.',
    'dev.responseLabel': 'Javob',
    'dev.rateLimit': 'Limit: daqiqasiga 60 ta soʻrov (har bir IP uchun).',
    'nav.files': 'Fayllar',
    'nav.apps': 'Ilovalar',
    'reform.title': '2026-yilgi islohot',
    'reform.intro': 'Oʻzbekiston 2026-yilda lotin alifbosini isloh qildi: 1995-yildagi qoʻsh harflar va apostrofli harflar bitta harfga aylandi.',
    'reform.changesLabel': 'Harf oʻzgarishlari',
    'reform.spotlightsLabel': 'Harfma-harf',
    'reform.sourcesLabel': 'Manbalar',
    'reform.law': 'Qonunchilik palatasi 2026-yil 7-iyulda qabul qildi, Senat 2026-yil 10-sentabrda maʼqulladi; endi Prezident imzosi kutilmoqda. Alifboda 28 ta harf va bitta tutuq belgisi bor (avval 26 ta harf va 3 ta harf birikmasi edi).',
    'reform.why': 'Bitta harflar noqulay qoʻsh harf va apostroflarni yoʻqotadi — ular dastur, URL va qidiruvni buzardi — hamda oʻzbekchani boshqa turkiy lotin alifbolariga yaqinlashtiradi.',
    'footer.reform': 'Islohot',
    'footer.developers': 'Dasturchilar',
    'footer.telegram': 'Telegram',
    'meta.home.title': 'Alfavit — yangi lotin yozuvi oʻgirgichi',
    'meta.home.desc': 'Oʻzbek matnini kirill yoki eski lotindan 2026-yilgi yangi lotinga oʻgiring — bepul, bir zumda, qurilmangizda.',
    'meta.files.title': 'Fayllarni oʻgirish — Alfavit',
    'meta.files.desc': '.txt, .srt va .docx fayllarni yangi oʻzbek lotiniga oʻgiring — brauzeringizda, hech narsa yuklanmaydi.',
    'meta.apps.title': 'Ilovalar va kanallar — Alfavit',
    'meta.apps.desc': 'Alfavit — veb ilova, Telegram bot, brauzer kengaytmasi va ochiq API. Bir tizim, har bir platforma.',
    'meta.dev.title': 'API va SDK — Alfavit',
    'meta.dev.desc': 'Oʻzbek matnini yangi lotinga oʻgirish uchun bepul ochiq API — kalit shart emas.',
    'meta.reform.title': '2026-yilgi alifbo islohoti — Alfavit',
    'meta.reform.desc': 'Oʻzbekistonning 2026-yilgi lotin alifbosi islohoti harfma-harf: nima oʻzgardi, Senat 2026-yil 10-sentabrda nimani maʼqulladi va oʻtish bosqichlari.',
    'meta.privacy.title': 'Maxfiylik — Alfavit',
    'meta.privacy.desc': 'Alfavit toʻliq qurilmangizda ishlaydi. Biz hech qanday maʼlumot toʻplamaymiz.',
    'faq.title': 'Koʻp beriladigan savollar',
    'meta.guide.cyrillic.title': 'Kirilldan lotinga oʻgirish — Alfavit',
    'meta.guide.cyrillic.desc': 'Kirilldagi oʻzbek matnini 2026-yilgi yangi lotin yozuviga qadamma-qadam oʻgiring.',
    'meta.guide.oldlatin.title': 'Eski lotindan yangi lotinga — Alfavit',
    'meta.guide.oldlatin.desc': '1995-yilgi qoʻsh harf va apostrofli harflarni yangi lotinga qadamma-qadam oʻgiring.',
    'guides.label': 'Qoʻllanmalar',
    'guides.cyrillic': 'Kirilldan lotinga',
    'guides.oldlatin': 'Eski lotindan yangi lotinga',
    'meta.alphabet.title': 'Oʻzbek alifbosi (2026) — toʻliq yangilangan lotin jadvali | Alfavit',
    'meta.alphabet.desc': '2026-yilgi yangilangan oʻzbek lotin alifbosi: barcha 28 harf eski lotin (1995) va kirill muqobillari, talaffuzi hamda misollari bilan.',
    'nav.alphabet': 'Alifbo',
    'footer.alphabet': 'Alifbo',
    'alphabet.title': 'Oʻzbek alifbosi',
    'alphabet.intro': '2026-yilgi yangilangan oʻzbek lotin alifbosida 28 ta harf va tutuq belgisi (ʼ) bor. Har bir harf eski lotin (1995) shakli, kirill muqobili, tovushi va misol bilan berilgan.',
    'alphabet.col.new': 'Yangi (2026)',
    'alphabet.col.old': 'Eski lotin (1995)',
    'alphabet.col.cyrillic': 'Kirill',
    'alphabet.col.sound': 'Tovush',
    'alphabet.col.example': 'Misol',
    'alphabet.changedLabel': 'Oʻzgargan',
    'alphabet.tutuq.label': 'Tutuq belgisi (ʼ)',
    'alphabet.tutuq.note': 'Tutuq belgisi — unlilarni ajratadi va boʻgʻiz toʻxtamini bildiradi. U 28 harf tarkibiga kirmaydi.',
    'alphabet.loanword.note': 'Oʻzlashma soʻzlarda kirillcha ц harfi s bilan yoziladi; Alfavit c variantini ham belgilab koʻrsatadi — tanlov sizda.',
    'alphabet.chart.heading': 'Alifbo jadvalini yuklab oling',
    'alphabet.chart.caption': 'A4 formatda, chop etishga tayyor — 28 ta harf, oʻzgargan toʻrttasi belgilangan.',
    'alphabet.chart.alt': 'Oʻzbek lotin alifbosi jadvali (2026): 28 ta harf eski lotin (1995) va kirill muqobillari, tovushi va misollari bilan',
    'alphabet.chart.png': 'PNG rasm',
    'alphabet.chart.pdf': 'A4 PDF',
    'notfound.title': 'Sahifa topilmadi',
    'notfound.body': 'Bu manzil alfavit.uz saytida mavjud emas. Xato yozilgan yoki havola oʻzgargan boʻlishi mumkin.',
    'meta.status.title': 'Alifbo oʻzgardimi? Yangi alifbo tasdiqlandimi — Alfavit',
    'meta.status.desc': '2026-yilgi alifbo islohoti kuchga kirdimi: qonun 2026-yil 10-sentabrda Senatdan oʻtdi va Prezident imzosini kutmoqda. Sana bilan tekshirilgan.',
    'status.title': 'Alifbo oʻzgardimi?',
    'status.checked': 'Holat tekshirilgan sana:',
    'status.timeline': 'Qanday boʻldi va keyin nima boʻladi',
    'status.sources': 'Manbalar',
    'alphabet.count.heading': 'Oʻzbek alifbosida nechta harf bor?',
    'alphabet.count.body': '2026-yilgi yangilangan oʻzbek lotin alifbosida 28 ta harf bor, ustiga tutuq belgisi (ʼ) — u harf emas, belgi. 1995-yilgi alifboda 26 ta harf va 3 ta harf birikmasi (sh, ch, ng) bor edi; islohot sh, ch, gʻ va oʻ ni bitta harfga — ş, ç, ğ va ö ga aylantirdi, ng esa alohida harf sifatida sanalmay qoʻydi.',
    'reform.timelineLabel': 'Bosqichlar',
    'reform.updated': '2026-yil 10-sentabrda yangilandi',
    'reform.ngNote': 'ng birikmasi endi alifboda alohida harf sifatida berilmaydi; imloda n + g tarzida yoziladi.',
    'news.senate': '10-sentabr: Senat 28 harfli yangi alifboni maʼqulladi. Nima oʻzgaradi →',
    'meta.guide.keyboard.title': 'Ş, Ç, Ö, Ğ harflarini qanday yozish — Alfavit',
    'meta.guide.keyboard.desc': 'Yangi toʻrt harfni Mac, Windows, iPhone va Android da yozish: nusxalash tugmalari, klaviatura joylashuvlari, tezkor tugmalar — yoki Alfavit yozayotganingizda oʻgirsin.',
    'guides.keyboard': 'ş ç ö ğ harflarini yozish',
    'converter.share': 'Ulashish',
  },
  ru: {
    'nav.convert': 'Конвертер',
    'nav.reform': 'Реформа',
    'nav.developers': 'Разработчикам',
    'nav.telegram': 'Telegram',
    'nav.reach': 'Контакты',
    'nav.cta': 'Начать',
    'hero.headlinePre': 'Ваши слова — в ',
    'hero.headlineEm': 'новом алфавите',
    'hero.headlinePost': ' Узбекистана.',
    'hero.desc': 'Мгновенно конвертируйте узбекский текст с кириллицы или старой латиницы в реформированную латиницу 2026 года — бесплатно, в браузере, ничего не загружается.',
    'converter.inputLabel': 'Кириллица или старая латиница',
    'converter.outputLabel': 'Новая латиница',
    'converter.placeholder': 'Введите текст…',
    'converter.copy': 'Копировать',
    'converter.copied': 'Скопировано',
    'converter.detected': 'Определено',
    'script.cyrillic': 'Кириллица',
    'script.old-latin': 'Старая латиница',
    'script.foreign': 'Текст',
    'channels.title': 'Alfavit — везде.',
    'channels.subtitle': 'Один движок — на любой платформе, где вы пишете.',
    'channels.group.desktop': 'Десктоп',
    'channels.group.mobile': 'Мобильные',
    'channels.group.more': 'Другие способы',
    'channels.open': 'Открыть',
    'channels.download': 'Скачать',
    'channels.mac.desc': 'Печатайте где угодно — кириллица или старая латиница сразу превращается в новую латиницу.',
    'channels.mac.note': 'Без подписи — если первый запуск заблокирован, разрешите его в «Системные настройки → Конфиденциальность и безопасность».',
    'channels.windows.desc': 'Печатайте где угодно — кириллица или старая латиница сразу превращается в новую латиницу.',
    'channels.windows.note': 'Без подписи — если SmartScreen блокирует первый запуск, нажмите «Подробнее → Выполнить в любом случае».',
    'channels.badge.new': 'Новое',
    'channels.web.name': 'Веб-приложение',
    'channels.telegram.name': 'Telegram-бот',
    'channels.extension.name': 'Расширение браузера',
    'channels.api.name': 'API и SDK',
    'channels.mac.name': 'macOS',
    'channels.windows.name': 'Windows',
    'channels.ios.name': 'iOS',
    'channels.android.name': 'Android',
    'status.soon': 'Скоро',
    'file.title': 'Конвертировать файл',
    'file.instruction': 'Файл конвертируется в браузере — ничего не загружается.',
    'file.choose': 'Выберите файл (.txt, .srt, .docx)',
    'file.convert': 'Конвертировать и скачать',
    'file.converting': 'Конвертация…',
    'file.done': 'Готово — загрузка начнётся.',
    'file.unsupported': 'Неподдерживаемый тип файла. Используйте .txt, .srt или .docx.',
    'file.error': 'Не удалось конвертировать файл. Попробуйте другой файл.',
    'dev.title': 'Разработчикам',
    'dev.intro': 'Бесплатный публичный API для конвертации узбекского текста в новую латиницу — ключ не нужен.',
    'dev.responseLabel': 'Ответ',
    'dev.rateLimit': 'Лимит: 60 запросов в минуту на IP.',
    'nav.files': 'Файлы',
    'nav.apps': 'Приложения',
    'reform.title': 'Реформа 2026 года',
    'reform.intro': 'В 2026 году Узбекистан реформировал латиницу: диграфы и буквы с апострофом образца 1995 года заменены одиночными буквами.',
    'reform.changesLabel': 'Изменения букв',
    'reform.spotlightsLabel': 'Буква за буквой',
    'reform.sourcesLabel': 'Источники',
    'reform.law': 'Принят Законодательной палатой 7 июля 2026 года и одобрен Сенатом 10 сентября 2026 года; теперь ожидает подписи Президента. В алфавите 28 букв и один знак апострофа (было 26 букв и 3 буквосочетания).',
    'reform.why': 'Одиночные буквы убирают неудобные диграфы и апострофы — которые ломали ПО, URL и поиск — и сближают узбекскую латиницу с другими тюркскими.',
    'footer.reform': 'Реформа',
    'footer.developers': 'Разработчикам',
    'footer.telegram': 'Telegram',
    'meta.home.title': 'Alfavit — конвертер узбекской новой латиницы',
    'meta.home.desc': 'Конвертируйте узбекский текст с кириллицы или старой латиницы в новую латиницу 2026 года — бесплатно, мгновенно, в браузере.',
    'meta.files.title': 'Конвертация файлов — Alfavit',
    'meta.files.desc': 'Конвертируйте файлы .txt, .srt и .docx в новую узбекскую латиницу — в браузере, ничего не загружается.',
    'meta.apps.title': 'Приложения и каналы — Alfavit',
    'meta.apps.desc': 'Alfavit — веб-приложение, Telegram-бот, расширение браузера и публичный API. Один движок, все платформы.',
    'meta.dev.title': 'API и SDK — Alfavit',
    'meta.dev.desc': 'Бесплатный публичный API для конвертации узбекского текста в новую латиницу — ключ не нужен.',
    'meta.reform.title': 'Реформа алфавита 2026 — Alfavit',
    'meta.reform.desc': 'Реформа узбекской латиницы 2026 года буква за буквой: что изменилось, что одобрил Сенат 10 сентября 2026 года и этапы перехода.',
    'meta.privacy.title': 'Конфиденциальность — Alfavit',
    'meta.privacy.desc': 'Alfavit работает полностью на вашем устройстве. Мы не собираем данные.',
    'faq.title': 'Часто задаваемые вопросы',
    'meta.guide.cyrillic.title': 'Кириллица в новую латиницу — Alfavit',
    'meta.guide.cyrillic.desc': 'Пошаговая конвертация узбекского текста с кириллицы в новую латиницу 2026.',
    'meta.guide.oldlatin.title': 'Старая латиница в новую — Alfavit',
    'meta.guide.oldlatin.desc': 'Пошаговая конвертация диграфов и букв с апострофом 1995 года в новую латиницу.',
    'guides.label': 'Руководства',
    'guides.cyrillic': 'Кириллица → латиница',
    'guides.oldlatin': 'Старая латиница → новая',
    'meta.alphabet.title': 'Узбекский алфавит (2026) — полная реформированная латиница | Alfavit',
    'meta.alphabet.desc': 'Полный реформированный узбекский латинский алфавит 2026 года: все 28 букв со старой латиницей (1995) и кириллическими соответствиями, произношением и примерами.',
    'nav.alphabet': 'Алфавит',
    'footer.alphabet': 'Алфавит',
    'alphabet.title': 'Узбекский алфавит',
    'alphabet.intro': 'В реформированном узбекском латинском алфавите 2026 года 28 букв и тутук белгиси (ʼ). Каждая буква показана со старой латиницей (1995), кириллицей, произношением и примером.',
    'alphabet.col.new': 'Новая (2026)',
    'alphabet.col.old': 'Старая латиница (1995)',
    'alphabet.col.cyrillic': 'Кириллица',
    'alphabet.col.sound': 'Звук',
    'alphabet.col.example': 'Пример',
    'alphabet.changedLabel': 'Изменена',
    'alphabet.tutuq.label': 'Тутук белгиси (ʼ)',
    'alphabet.tutuq.note': 'Знак апострофа — разделяет гласные и обозначает гортанную смычку. Не входит в число 28 букв.',
    'alphabet.loanword.note': 'В заимствованиях кириллическая ц передаётся как s; Alfavit отмечает c как альтернативу, чтобы вы могли выбрать.',
    'alphabet.chart.heading': 'Скачать таблицу алфавита',
    'alphabet.chart.caption': 'Формат A4, готово к печати — 28 букв, четыре изменённые отмечены.',
    'alphabet.chart.alt': 'Таблица узбекского латинского алфавита (2026): 28 букв со старой латиницей (1995), кириллицей, произношением и примерами',
    'alphabet.chart.png': 'PNG-изображение',
    'alphabet.chart.pdf': 'A4 PDF',
    'notfound.title': 'Страница не найдена',
    'notfound.body': 'Такого адреса на alfavit.uz нет. Возможно, опечатка или ссылка изменилась.',
    'meta.status.title': 'Узбекский алфавит уже изменился? — Alfavit',
    'meta.status.desc': 'Вступила ли в силу реформа узбекского алфавита 2026 года: закон одобрен Сенатом 10 сентября 2026 года и ждёт подписи Президента. С датой проверки.',
    'status.title': 'Алфавит уже изменился?',
    'status.checked': 'Статус проверен:',
    'status.timeline': 'Как к этому пришли и что дальше',
    'status.sources': 'Источники',
    'alphabet.count.heading': 'Сколько букв в узбекском алфавите?',
    'alphabet.count.body': 'В реформированном узбекском латинском алфавите 2026 года 28 букв плюс тутук белгиси (ʼ) — это знак, а не буква. В алфавите 1995 года было 26 букв и 3 диграфа (sh, ch, ng); реформа превратила sh, ch, gʻ и oʻ в отдельные буквы ş, ç, ğ и ö, а ng перестал считаться самостоятельной буквой.',
    'reform.timelineLabel': 'Хронология',
    'reform.updated': 'Обновлено 10 сентября 2026 года',
    'reform.ngNote': 'Сочетание ng больше не считается отдельной буквой; в написании остаётся как n + g.',
    'news.senate': '10 сентября: Сенат одобрил алфавит из 28 букв. Что меняется →',
    'meta.guide.keyboard.title': 'Как набирать Ş, Ç, Ö, Ğ — Alfavit',
    'meta.guide.keyboard.desc': 'Четыре новые узбекские буквы на Mac, Windows, iPhone и Android: кнопки копирования, раскладки, сочетания клавиш — или пусть Alfavit конвертирует по мере ввода.',
    'guides.keyboard': 'Как набирать ş ç ö ğ',
    'converter.share': 'Поделиться',
  },
} as const

export type TranslationKey = keyof typeof translations.en

export const LOCALES: readonly Locale[] = ['uz', 'ru', 'en']
export const DEFAULT_LOCALE: Locale = 'uz'
